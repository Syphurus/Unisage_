...
function mapPgError(err, fallbackMessage) {
  ...
  if (typeof message === "string") {
    ...
    if (message.includes("version conflict") || code === "40001") {
      return new ConflictError("Payment was modified by another action — refresh and retry");
    }
...
async function listActivePlans() {
  const { data, error } = await supabase
    .from("plans")
    .select("id, code, name, description, amount_inr_paise, duration_days, scopes")
    .eq("is_active", true)
...
async function getPlanById(planId) {
  const { data, error } = await supabase
    .from("plans")
    .select("id, code, name, description, amount_inr_paise, duration_days, scopes, is_active")
    .eq("id", planId)
    .maybeSingle();
  if (error) throw error;
...
async function createIntent({ user, planId, req }) {
  ...
  const intentExpiresAt = new Date(Date.now() + intentTtlHours * 60 * 60 * 1000).toISOString();
  ...
  const { data: inserted, error } = await supabase
    ...
    .select("id, reference_code, status, amount_inr_paise, scopes, duration_days, intent_expires_at, created_at, version")
    .single();
...
async function submitProof({ user, paymentId, utr, upload, req }) {
  ...
  if (error) {
    ...
    if (error.code === "23505") {
      ...
      throw new ConflictError("This UTR is already associated with another live payment");
    }
...
async function revoke({ admin, paymentId, expectedVersion, reason, req }) {
  ...
  await audit.record({
    ...
    targetType: "payment",
    targetId: paymentId,
    metadata: { reason: reason.trim(), entitlements_revoked: data?.entitlements_revoked || 0 },
  });
...
async function adminListQueue({ status, query, limit = 50, offset = 0 }) {
  let q = supabase.from("payments").select(ADMIN_PAYMENT_FIELDS, { count: "exact" });
...
async function adminGetPayment({ paymentId }) {
  ...
  if (data.proof_upload_id) {
    const { data: u } = await supabase
      ...
      .select("id, storage_bucket, storage_key, mime_type, size_bytes, sha256, uploaded_at")
